...
IBtable = data.table::fread("IBtable.csv")
```
...
regIBipDemo_fig <- data.frame(summary(regIBipDemo)$coefficients[2:4,1:4]) %>% 
    rownames_to_column(var = "Type") %>% dplyr::mutate(Label = "Ip")
...
regIBifdiffDemo_fig <- data.frame(summary(regIBifdiffDemo)$coefficients[2:4,1:4]) %>% rownames_to_column(var = "Type") %>% dplyr::mutate(Label = "Ifdiff")


regDemo_fig <- rbind(regAccuracyDemo_fig,regConfidenceDemo_fig,
                     regIBipDemo_fig,regIBdtbDemo_fig,
                     regIBifratioDemo_fig,regIBifdiffDemo_fig)
...
regIBipTrait <- generic.model("Ipast_task_sc", summedscaledData) %>% rownames_to_column(var = "Type") %>% dplyr::mutate(Label = "Ip")
...
regIBifdiffTrait <- generic.model("diff_sc", summedscaledData) %>% rownames_to_column(var = "Type") %>% dplyr::mutate(Label = "Ifdiff")


regTrait_fig<-rbind(regAccuracyTrait,regConfidenceTrait,
                    regIBipTrait,regIBdtbTrait,
                    regIBifratioTrait,regIBifdiffTrait)
...
```{r}
nCng(het.mat1, cor = TRUE, model = "components", details = TRUE)
...
regIBifFactor = lm(Ipast_task_sc ~ ML1 + ML2 + ML3 + ML4 + ML5 + Age_sc + Gender,factorDataV) 
regIBdtbFactor = lm(DTB_sc ~ ML1 + ML2 + ML3 + ML4 + ML5 + Age_sc + Gender,factorDataV)
regIBifratioFactor = lm(ratio_sc ~ ML1 + ML2 + ML3 + ML4 + ML5 + Age_sc + Gender,factorDataV)
regIBifdiffFactor = lm(diff_sc ~ ML1 + ML2 + ML3 + ML4 + ML5 + Age_sc + Gender,factorDataV)
...
regIBifFactor_fig <- data.frame(summary(regIBifFactor)$coefficients[-1,]) %>% rownames_to_column(var = "Type") %>% dplyr::mutate(Label = "Ip")
...
regIBifdiffFactor_fig <- data.frame(summary(regIBifdiffFactor)$coefficients[-1,]) %>% rownames_to_column(var = "Type") %>% dplyr::mutate(Label = "Ifdiff")

regFactor_fig <- rbind(regAccuracyFactor_fig,regConfidenceFactor_fig,
                       regIBifFactor_fig,regIBdtbFactor_fig,
                       regIBifratioFactor_fig,regIBifdiffFactor_fig)
